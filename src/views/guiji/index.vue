  ...
  export default {
    ...
    methods: {
      ...
      searchHandler(){
        ...
        //let loadingInstance = Loading.service({ fullscreen: true,text:"载入数据"});
        $.ajax({
          url: window.GJCONFIG.localhost+"/PositionService.svc/GetList",
      ...
      formatData(data){
        ...
        var data = {
          ...
          GPSTime:String(data.A),//GPS 日期时间
          GPSLat:data.C,//GPS 纬度
          GPSLng:data.B,//GPS 经度
          GPSRotate:data.E,//GPS 方向
          GPSSpeed:data.D,//GPS 速度
          GPSStatus:data.F,//GPS 无效定位
          RouteId:data.G,//线路 ID（跑法号）
          Updown:data.H,//上下行
          StationNumber:data.I,//站点顺序号
        }
        return data;
      },
